...
import axel.modules.sql.antispam_sql as sql
from axel import dispatcher, OWNER_ID, SUDO_USERS, SUPPORT_USERS, STRICT_GBAN
from axel.modules.helper_funcs.chat_status import user_admin, is_user_admin
...
from axel.modules.sql.users_sql import get_all_chats
...
def check_and_ban(update, user_id, should_message=True):
    if sql.is_user_gbanned(user_id):
        ...
        if should_message:
            update.effective_message.reply_text("Spammer Detected!!!")
...
def gbanstat(bot: Bot, update: Update, args: List[str]):
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text("I've enabled antispam in this group. This will help protect you "
                                                "from spammers, unsavoury characters, and the biggest trolls.")
        elif args[0].lower() in ["off", "no"]:
            sql.disable_gbans(update.effective_chat.id)
            update.effective_message.reply_text("I've disabled antispam in this group. GBans wont affect your users "
