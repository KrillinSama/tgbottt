...
from axel import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from axel.modules.helper_funcs.handlers import CMD_STARTERS
from axel.modules.helper_funcs.misc import is_module_loaded, send_to_list
from axel.modules.helper_funcs.chat_status import is_user_admin
from axel.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from axel.modules.helper_funcs.string_handling import markdown_parser
from axel.modules.disable import DisableAbleCommandHandler

import axel.modules.sql.feds_sql as sql
...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text("Couldn't create your federation! ")
            return

        update.effective_message.reply_text("*federation has been created!*"\
                                            "\nName: `{}`"\
                                            "\nID: `{}`"
                                            "\n\nUse command below to join the federation:"
                                            "\n`/joinfed {}`".format(fed_name, fed_id, fed_id), parse_mode=ParseMode.MARKDOWN)
        bot.send_message(
            MESSAGE_DUMP, OWNER_ID,
...
def del_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if not fed_id:
            update.effective_message.reply_text("delete federation on the group that joined it.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
        ...
        for admin in administrators:
            ...
            if status == "creator":
                ...
                if str(admin.user.id) == str(user.id):
                    ...
                    return
    if fed_id:
        message.reply_text("Groups, only allowed to join one federation!")
        return
    ...
    if len(args) >= 1:
        ...
        if not x:
                message.reply_text("Failed to join to federation!")
                return
...
def leave_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if sql.chat_leave_fed(chat.id) == True:
        update.effective_message.reply_text("Left from fed!")
    else:
        update.effective_message.reply_text("There is no any federation connected to this group already!")
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
        ...
        #if user_id == user_id:
        #        update.effective_message.reply_text("Are you gonna promote yourself?")
        #        return
        ...
        HAHA = OW.id
        if user_id == HAHA:
                update.effective_message.reply_text("Why are you trying to promote federation owner!?")
                return

        if not sql.search_user_in_fed(fed_id, user_id) == False:
                update.effective_message.reply_text("This user is already an admin in this federation!")
                return

        if user_id == bot.id:
                update.effective_message.reply_text("I am already the federation admin and the one that manage it!")
                return
        ...
        if not res:
                update.effective_message.reply_text("Failed to promoted!")
                return

        update.effective_message.reply_text("Promoted Successfully!")
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if user_id == bot.id:
                update.effective_message.reply_text("What are you trying to do? Demoting me from your federation?")
                return

        if sql.search_user_in_fed(fed_id, user_id) == False:
                update.effective_message.reply_text("I can't demote user which is not a fed admin! If you wanna bring him to tears, promote him first!")
                return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if not fban == False:
        update.effective_message.reply_text("*Cough* This user is already fbanned!")
        return
    ...
    if is_user_fed_owner(fed_id, user_id) == True:
        message.reply_text("Why you are trying to fban the federation owner?")
        return

    if is_user_fed_admin(fed_id, user_id) == True:
        message.reply_text("Why so serious trying to fban the federation admin?")
        return

    if user_id == OWNER_ID:
        message.reply_text("I'm not fbanning my master, That's pretty dumb idea!")
        return

    if int(user_id) in SUDO_USERS:
        message.reply_text("I'm not fbanning the bot sudoers!")
        return

    if int(user_id) in WHITELIST_USERS:
        message.reply_text("This person is whitelisted from being fbanned!")
        return
...
def unfban(bot: Bot, update: Update, args: List[str]):
    ...
    for O in h:
        ...
        try:
            ...
            if not x:
                message.reply_text("Failed to fban, This user is probably fbanned!")
                return
        except:
            pass
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        update.effective_message.reply_text(f"Rules are set for {rules}!")
    else:
...
def get_frules(bot: Bot, update: Update, args: List[str]):
    ...
    try:
        ...
        update.effective_message.reply_text("There are no rules in this federation!")
        return
...
__help__ = """
Federations is your Own GBAN list, Protect all of your groups at same time!
